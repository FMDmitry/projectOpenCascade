...
namespace FileDialog {
	...
	void ShowFileDialog(bool* open, char* buffer, [[maybe_unused]] unsigned int buffer_size, FileDialogType type = FileDialogType::OpenFile) {
		...
		if (open) {
            ...
            const char* window_title = (type == FileDialogType::OpenFile ? "Выбрать файл" : "Выбрать папку");
            ...
            ImGui::BeginChild("Директории##1", ImVec2(200, 300), true, ImGuiWindowFlags_HorizontalScrollbar);
			...
			for (int i = 0; i < folders.size(); ++i) {
                if (ImGui::Selectable(folders[i].path().stem().string().c_str(), i == file_dialog_folder_select_index, ImGuiSelectableFlags_AllowDoubleClick)) {
            ...
            ImGui::BeginChild("Файлы##1", ImVec2(516, 300), true, ImGuiWindowFlags_HorizontalScrollbar);
			ImGui::Columns(4);
			...
			for (int i = 0; i < files.size(); ++i) {
                if (ImGui::Selectable(files[i].path().filename().string().c_str(), i == file_dialog_file_select_index, ImGuiSelectableFlags_AllowDoubleClick)) {
			...
			if (ImGui::BeginPopup("NewFolderPopup", ImGuiWindowFlags_Modal)) {
                ImGui::Text("Введите название новой папки");
                ...
                if (ImGui::Button("Создать##1")) {
					if (strlen(new_folder_name) <= 0) {
                        strcpy_s(new_folder_error, "Имя папки недопустимо");
					}
					else {
			...
			if (ImGui::BeginPopup("DeleteFolderPopup", ImGuiWindowFlags_Modal)) {
                ImGui::TextColored(ImColor(1.0f, 0.0f, 0.2f, 1.0f), "Вы уверены, что хотите удалить эту папку?");
				ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 6);
            ...
            if (ImGui::Button("Выбрать")) {
                if (type == FileDialogType::SelectFolder) {
					if (file_dialog_current_folder == "") {
                        strcpy_s(file_dialog_error, "Ошибка: Вы должны выбрать папку!");
					}
					else {
                        auto path = file_dialog_current_path + (file_dialog_current_path.back() == '\\' ? "" : "\\") + file_dialog_current_folder + (file_dialog_current_folder.back() == '\\' ? "" : "\\");
					...
					if (file_dialog_current_file == "") {
                        strcpy_s(file_dialog_error, "Ошибка: Вы должны выбрать файл!");
					}
					else {
